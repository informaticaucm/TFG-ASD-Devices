...
esp_err_t lvgl_port_init(const lvgl_port_cfg_t *cfg)
{
    ...
    BaseType_t res;
    StackType_t *const puxStackBuffer = heap_caps_malloc(cfg->task_stack * sizeof(StackType_t), MALLOC_CAP_SPIRAM);
    StaticTask_t *const pxTaskBuffer = heap_caps_malloc(sizeof(StaticTask_t), MALLOC_CAP_INTERNAL);
    if (cfg->task_affinity < 0)
    {
        // res = xTaskCreate(lvgl_port_task, "LVGL task", cfg->task_stack, NULL, cfg->task_priority, NULL);
        TaskHandle_t handler = xTaskCreateStatic(lvgl_port_task, "LVGL task", cfg->task_stack, NULL, cfg->task_priority, puxStackBuffer, pxTaskBuffer);
        res = handler == NULL ? pdFAIL : pdPASS;
    }
    else
    {
        // res = xTaskCreatePinnedToCore(lvgl_port_task, "LVGL task", cfg->task_stack, NULL, cfg->task_priority, NULL, cfg->task_affinity);
        TaskHandle_t handler = xTaskCreateStaticPinnedToCore(lvgl_port_task, "LVGL task", cfg->task_stack, NULL, cfg->task_priority, puxStackBuffer, pxTaskBuffer, cfg->task_affinity);
        res = handler == NULL ? pdFAIL : pdPASS;
    }
